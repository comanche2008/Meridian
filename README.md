...
> 安装脚本从 GitHub Releases 下载二进制，并使用同一 Release 中的 `SHA256SUMS` 强制校验。systemd 部署默认使用独立的 `meridian` 非 root 用户。
...
bash <(curl -fsSL https://raw.githubusercontent.com/snnabb/Meridian/master/install.sh) install v1.4.3
...
# 状态、重启、日志和一致性备份
bash <(curl -fsSL https://raw.githubusercontent.com/snnabb/Meridian/master/install.sh) status
bash <(curl -fsSL https://raw.githubusercontent.com/snnabb/Meridian/master/install.sh) restart
bash <(curl -fsSL https://raw.githubusercontent.com/snnabb/Meridian/master/install.sh) logs 200
bash <(curl -fsSL https://raw.githubusercontent.com/snnabb/Meridian/master/install.sh) backup

# 回滚到上一版本二进制；回滚前也会先备份数据
bash <(curl -fsSL https://raw.githubusercontent.com/snnabb/Meridian/master/install.sh) rollback

# 卸载默认保留数据；只有显式添加 --purge 才删除数据
bash <(curl -fsSL https://raw.githubusercontent.com/snnabb/Meridian/master/install.sh) uninstall
```

其他常用参数：`-y`/`--yes` 用于非交互确认，`logs --follow` 持续跟踪日志，`logs --lines 500` 指定日志行数。备份默认保存在 `/opt/meridian-backups`，权限为 `0600`，其中包含数据库、JWT 密钥和初始化配置，请按敏感文件保管。
...
部署完成后访问 `http://你的IP:9090`。首次打开会要求输入管理员账号、至少 12 位的密码，以及启动日志中显示的初始化令牌。也可以预先设置 `SETUP_TOKEN` 环境变量。
...
一键创建一致性备份（systemd 部署会短暂停止服务，完成后自动启动并检查健康状态）：
